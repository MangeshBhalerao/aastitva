...
function Allhoodie() {
  ...
  const designs = [...new Set(Hoodieproducts.filter(product => product.design).map(product => product.design))]
  
  // Initialize filtered products with all Hoodie products
  useEffect(() => {
    setFilteredProducts(Hoodieproducts.filter(product => product.category === 'Hoodie'))
    setIsLoading(false)
  }, [])
    ...
    <div className='w-full min-h-screen bg-[#0D0D0D] text-white p-2 sm:p-4 pt-28' style={{
      ...
      <div className='flex flex-col md:flex-row gap-4'>
        ...
        <div className='flex-1'>
          ...
          <div className="mb-4 lg:mt-20 flex justify-between items-center ml-2">
            <h2 className="text-2xl font-bold">
              {isLoading ? 'Loading products...' : `Hoodies (${filteredProducts.length})`}
            </h2>
          </div>
