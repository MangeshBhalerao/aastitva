...
function Alltshirt() {
  ...
  const categories = [...new Set(Tshirtproducts.map(product => product.category))]
  const designs = [...new Set(Tshirtproducts.filter(product => product.design).map(product => product.design))]
  
  // Initialize filtered products with all Tshirt products
  useEffect(() => {
    setFilteredProducts(Tshirtproducts.filter(product => product.category === 'Tshirt'))
    setIsLoading(false)
  }, [])
        ...
        <div className='flex-1'>
          ...
          <div className="mb-4 mt-20 flex justify-between items-center">
            <h2 className="text-2xl font-bold">
              {isLoading ? 'Loading products...' : `T-Shirts (${filteredProducts.length})`}
            </h2>
          </div>
