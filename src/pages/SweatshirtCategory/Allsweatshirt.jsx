...
function Allsweatshirt() {
  ...
  return (
    <div className='w-full min-h-screen bg-[#0D0D0D] text-white p-4' style={{
      ...
      <h1 className='text-5xl text-center text-white font-thin mb-8'>All Sweatshirts</h1>
      <div className='flex'>
        ...
        <div className='w-3/4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-8 justify-center'>
          {allProducts.map((product, index) => (
            <div key={index} className='max-w-sm py-2 rounded overflow-hidden shadow-lg bg-[#0D0D0D] text-white m-4'>
              <div className='w-full aspect-w-1 aspect-h-1'>
                <img className='w-full h-full object-cover' src={product.image} alt={product.title} />
              </div>
              <div className='px-6 py-4'>
                <div className='font-bold text-3xl mb-2 text-[#FF0000]'>{product.title}</div>
